...
int main(){
    std::cout << "Testing slices of expressions\n";
    ...
    if (any((a+1)(Range(1,2))!=literal<int>(2,3))){
        std::cout << "Error in 1D contiguous slice starting at LB\n";
        return -1;
    }

    if (any((a+1)(Range(2,3))!=literal<int>(3,4))){
        std::cout << "Error in 1D contiguous slice ending at UB\n";
        return -1;
    }

    if (any((a+1)(Range(1,3,2))!=literal<int>(2,4))){
        std::cout << "Error in 1D non-contiguous slice with stride\n";
        return -1;
    }

    if (any((a+1)(Range(3,1,-1))!=literal<int>(4,3,2))){
        std::cout << "Error in 1D non-contiguous slice with negative stride\n";
        return -1;
    }

    if (any((a+1)(Range(3,1,-2))!=literal<int>(4,2))){
        std::cout << "Error in 1D non-contiguous slice with negative stride\n";
        return -1;
    }

    Array<int,2> b={{1,2,3},{4,5,6},{7,8,9}};
    if (any((b+1)(Range(),Range())!=(b+1))){
        std::cout << "Whole 2D range != array\n";
        return -1;
    }

    if (any((b+1)(Range(1,2),Range(1,2))!=reshape(literal<int>(2,3,5,6),2,2))){
        std::cout << "2x2 slice starting at LB,LB wrong\n";
        return -1;
    }

    if (any((b+1)(Range(2,3),Range(1,2))!=reshape(literal<int>(3,4,6,7),2,2))){
        std::cout << "2x2 slice starting at UB,LB wrong\n";
        return -1;
    }

    if (any((b+1)(Range(1,2), Range(2,3))!=reshape(literal<int>(5,6,8,9),2,2))){
        std::cout << "2x2 slice starting at LB,UB wrong\n";
        return -1;
    }

    if (any((b+1)(Range(2,3), Range(2,3))!=reshape(literal<int>(6,7,9,10),2,2))){
        std::cout << "2x2 slice starting at UB,UB wrong\n";
        return -1;
    }

    if (any((b+1)(Range(1,3,2), Range(1,3,2))!=reshape(literal<int>(2,4,8,10),2,2))){
        std::cout << "2x2 non-contiguous slice with stride wrong\n";
        return -1;
    }

    if (any((b+1)(Range(1,3,2), Range(1,2))!=reshape(literal<int>(2,4,5,7),2,2))){
        std::cout << "2x2 non-contiguous slice with stride in one dimension wrong\n";
        return -1;
    }

    if (any((b+1)(Range(3,1,-1), Range(3,1,-1))!=reshape(literal<int>(10,9,8,7,6,5,4,3,2),3,3))){
        std::cout << "2x2 non-contiguous slice with negative stride wrong\n";
        return -1;
    }

    if (any((b+1)(Range(3,1,-1), Range(2,3))!=reshape(literal<int>(7,6,5,10,9,8),3,2))){
...

    {
        ...
        if (any(c!=reshape(literal<int>(2,3,4),3,1))){
