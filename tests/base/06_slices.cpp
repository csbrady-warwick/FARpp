...
int main(){
    ...
    if (any(b(Range(1,2),Range(1,2))!=reshape(literal<int>(1,2,4,5),2,2))){
        std::cout << "2x2 slice starting at LB,LB wrong\n";
        return -1;
    }

    if (any(b(Range(2,3),Range(1,2))!=reshape(literal<int>(2,3,5,6),2,2))){
        std::cout << "2x2 slice starting at UB,LB wrong\n";
        return -1;
    }

    if (any(b(Range(1,2), Range(2,3))!=reshape(literal<int>(4,5,7,8),2,2))){
        std::cout << "2x2 slice starting at LB,UB wrong\n";
        return -1;
    }

    if (any(b(Range(2,3), Range(2,3))!=reshape(literal<int>(5,6,8,9),2,2))){
        std::cout << "2x2 slice starting at UB,UB wrong\n";
        return -1;
    }

    if (any(b(Range(1,3,2), Range(1,3,2))!=reshape(literal<int>(1,3,7,9),2,2))){
        std::cout << "2x2 non-contiguous slice with stride wrong\n";
        return -1;
    }

    if (any(b(Range(1,3,2), Range(1,2))!=reshape(literal<int>(1,3,4,6),2,2))){
        std::cout << "2x2 non-contiguous slice with stride in one dimension wrong\n";
        return -1;
    }

    if (any(b(Range(3,1,-1), Range(3,1,-1))!=reshape(literal<int>(9,8,7,6,5,4,3,2,1),3,3))){
        std::cout << "2x2 non-contiguous slice with negative stride wrong\n";
        return -1;
    }

    if (any(b(Range(3,1,-1), Range(2,3))!=reshape(literal<int>(6,5,4,9,8,7),3,2))){
...

    {
        ...
        if (any(c!=reshape(literal<int>(1,2,3),3,1))){
